...
def get_selected_row(event):
    global selected_row
    index = lister.curselection()[0]
    selected_row = lister.get(index)
    e1.delete(0,END)
...
def add_command():
    ...
    lister.delete(0,END)
    lister.insert(END,(date_text.get(),earning_text.get(),exercise_text.get(),study_text.get(),diet_text.get(),python_text.get()))
...
lister = Listbox(win, height=8, width=35)
lister.grid(row=3, column=0, rowspan=9, columnspan=2)

# Scroll bar

scroller = Scrollbar(win)
scroller.grid(row=3, column=2, rowspan=9)
...
b5 = Button(win, text="EXIT", width=12, pady=5, command= win.destroy)
